...
export default function AuditPage() {
  const { data: logs, error } = useAuditLog();
          ...
          <tbody className="text-sm divide-y divide-zinc-800">
            {logs && logs.map((log: any, idx: number) => (
              <tr key={idx} className="hover:bg-zinc-800/50 transition-colors">
                ...
                <td className="p-4"><span className="px-2 py-0.5 rounded-full bg-blue-900/50 text-blue-200 text-xs font-semibold">{log.action}</span></td>
                <td className="p-4 truncate max-w-[200px]">{log.reason}</td>
                <td className="p-4">{log.replicasDiff > 0 ? `+${log.replicasDiff}` : log.replicasDiff}</td>
              </tr>
            ))}
            {!logs && (
              <tr><td colSpan={5} className="p-4 text-center text-zinc-500">No logs found or loading...</td></tr>
            )}
          </tbody>
        </table>
      </div>
